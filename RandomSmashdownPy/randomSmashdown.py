...
class FighterApp:
    def __init__(self, root):
        ...
        self.name_label = tk.Label(root, text="", font=("Impact", 24), bg="#888888", fg="white")
        self.name_label.pack(pady=10)

        self.icon_label = tk.Label(root, bg="#888888")
        self.icon_label.pack(pady=15)

        self.button_frame = tk.Frame(root, bg="#888888")
        self.button_frame.pack(pady=10)

        self.continue_btn = tk.Button(self.button_frame, text="Continue", command=self.next_fighter, width=10)
        self.continue_btn.grid(row=0, column=0, padx=5)
        self.end_btn = tk.Button(self.button_frame, text="End", command=self.end_app, width=10)
        self.end_btn.grid(row=0, column=1, padx=5)
        self.reset_btn = tk.Button(self.button_frame, text="Reset", command=self.reset_list, width=10)
    ...
    def next_fighter(self):
        if not self.fighters:
            self.name_label.config(text="No fighters left!")
            self.icon_label.config(image="")
            return
        self.selected_fighter = random.choice(self.fighters)
        self.name_label.config(text=self.selected_fighter["name"])
    ...
    def show_icon(self, url):
        try:
            ...
            self.icon_label.config(image="", text="[Icon not available]")
